...
addToBasketButtons.forEach((button) => {
	...
	button.addEventListener("click", (event) => {
		let currentEl = basket.querySelector(
			'.cart__product[data-id="' + productId + '"]'
		);
		if (currentEl) {
			currentEl.querySelector(".cart__product-count").innerText = `${
				parseInt(
					currentEl.querySelector(".cart__product-count").innerText,
					10
				) +
			...
			basket.insertAdjacentHTML('beforeend', createCartItem(event));
		}
	});
});
...
function createCartItem(event) {
	...
	elementId = event.target.closest(".product").dataset.id;
	elementSrc = event.target.closest(".product").querySelector(".product__image").src;
	elementCount = event.target.closest(".product").querySelector(".product__quantity-value").innerText;

	return `<div class="cart__product" data-id=${elementId}><img class="cart__product-image" src=${elementSrc}><div class="cart__product-count">${elementCount}</div></div>`;
}
